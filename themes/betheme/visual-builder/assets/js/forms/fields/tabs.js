function mfn_field_tabs(field) {
	...
	const inputs = {
		input: function input(val, o, i) {
			...
			return `<input data-order="${i}" data-label="${o}" class="${input_classes.join(' ')}" type="text" name="${field.id}[${i}][${o}]" value="${val}"/>`;
		},
		select: function select(val, o, i) {
			...
			if( typeof options[o][3] !== 'undefined' ){
